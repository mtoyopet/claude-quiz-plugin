...
Anthropic API を使い、**推定したセッショントピック**に基づいたクイズを毎回動的生成する。

- モデル: `claude-haiku-4-5-20251001`（高速・低コスト）
- トピックをプロンプトに含めてクイズを動的生成
- 静的問題バンクは廃止。問題は常に Anthropic API で生成する
- **フォールバック: API キー未設定時はエラーメッセージを表示し、クイズを出さない**
...
| `src/quizGenerator.ts` | トピックを受け取り Anthropic SDK でクイズ生成（常に動的生成） |
...
5. ~~`src/staticQuestions.ts` の静的問題バンク~~ → 廃止（Anthropic API で動的生成に一本化）
6. `src/quizPanel.ts` + `media/` の WebView UI
7. `src/quizGenerator.ts` の Anthropic API 連携（トピック連動、常に動的生成）
